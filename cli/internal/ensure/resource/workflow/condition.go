...
func (c *Condition) Ensure(ctx context.Context, input render.InputParams) error {
	...
	cmd.Stdout(ctx).Printf("Waiting on condition: %s.%s path %s matches %s (timeout: %s)", c.Namespace, c.Name, c.Jsonpath, c.Value, c.Timeout)
...
func (c *Condition) conditionMet(ctx context.Context, input render.InputParams) (bool, error) {
	...
	if err != nil {
		cmd.Stderr(ctx).Printf("Error checking condition")
		return false, err
	}
	if out == c.Value {
		cmd.Stdout(ctx).Printf("Condition met!")
		return true, nil
	}
	return false, nil
}
...
func (*Condition) Teardown(ctx context.Context, input render.InputParams) error {
	cmd.Stdout(ctx).Printf("Skipping teardown for condition")
	return nil
}
