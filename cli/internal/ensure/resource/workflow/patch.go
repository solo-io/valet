...
func (p *Patch) Ensure(ctx context.Context, input render.InputParams) error {
	...
	cmd.Stdout(ctx).Printf("Patching %s.%s (%s) from file %s (%s) %s", p.Namespace, p.Name, p.KubeType, p.Path, p.PatchType, input.Values.ToString())
...
func (p *Patch) Teardown(ctx context.Context, input render.InputParams) error {
	...
	cmd.Stdout(ctx).Printf("Skipping teardown for patch")
	return nil
}
