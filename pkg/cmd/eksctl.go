...
func (e *EksCtl) DeleteCluster(name, region string, runner Runner) error {
	streamHandler, err := runner.Stream(e.With("delete", "cluster").Region(region).WithName(name).Cmd())
	if err != nil {
		return err
	}
...
func (e *EksCtl) CreateCluster(name, region string, runner Runner) error {
	streamHandler, err := runner.Stream(e.With("create", "cluster").Region(region).WithName(name).Cmd())
	if err != nil {
		return err
	}
...
func (e *EksCtl) IsRunning(name, region string, runner Runner) (bool, error) {
	output, err := runner.Output(e.GetCluster().Region(region).Name(name).SwallowError().Cmd())
	if err != nil {
...
func (e *EksCtl) WriteKubeConfig(name, region string, runner Runner) error {
	output, err := runner.Output(e.GetCredentials().Region(region).Name(name).With("--auto-kubeconfig").SwallowError().Cmd())
	if err != nil {
